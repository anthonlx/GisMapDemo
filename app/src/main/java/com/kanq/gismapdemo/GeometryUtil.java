...
public class GeometryUtil {
	...
	public static void highlightGeometriesWithColor(Geometry[] results,
		...
		highlightGeometriesWithColor(results, graphicsLayer, color, 10);
	}
		...
		for (int i = 0; i < results.length; i++) {
			...
			if (typeName.equalsIgnoreCase("point")) {
				...
				addPolylineToGraphicsLayer((Polyline) geom, color, size / 4,
						graphicsLayer);
//				addPointToGraphicsLayer((Point) geom, graphicsLayer, color,
//						size, STYLE.CIRCLE);//不能直接这样，无法强制转换为Point
	 ...
	 * @param graphicsLayer
	 */
	public static void addPolygon(Polygon polygon,GraphicsLayer graphicsLayer){
		SimpleLineSymbol sms = new SimpleLineSymbol(Color.RED, 3, SimpleLineSymbol.STYLE.SOLID);
		Graphic gp = new Graphic(polygon, sms);
